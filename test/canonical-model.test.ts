import { CANONICAL_METADATA_TAGS, EFFECT_TYPES, IMPLEMENTED_EFFECT_TYPES, RESERVED_EFFECT_TYPES, assertAllEffectHandlersRegistered, compareAuthorityV1, compileLegacyAuthoringTag, createEffectExecutionServices, directlyConflicts, dispatchEffect, registeredEffectHandlerCount, requireCanonicalMetadataTag, resolveDirectConflictV1, type MechanicClaim } from '../src/combat/canonical-model';
...
test('canonical tag and effect registries are closed', () => {
  ...
  expect(registeredEffectHandlerCount()).toBe(IMPLEMENTED_EFFECT_TYPES.length);
  ...
  const commit = () => ({ eventSerial: 1, stateRevision: 1, actionId: 'a', chainId: 'c', targetLifeIds: ['x:1'] });
  ...
  expect(dispatchEffect(effect, context, 'x', 'kit.skill1.effect')).toEqual({ effectType: 'heal', committed: true, eventSerial: 1, stateRevision: 1, actionId: 'a', chainId: 'c', targetLifeIds: ['x:1'] });
  expect(compileLegacyAuthoringTag('sleep-setup', 'x', 'kit.basic.tags[0]')).toBe('cc:sleep');
  expect(() => compileLegacyAuthoringTag('invented-gameplay', 'x', 'kit.basic.tags[1]')).toThrow('kit.basic.tags[1]');
  for (const type of RESERVED_EFFECT_TYPES) expect(() => dispatchEffect({ type, target: { kind: 'self' }, payload: type === 'queue-delayed-revive' ? { effectId: 'x', duePolicy: 'next-natural-turn' } : type === 'trigger-follow-up' || type === 'trigger-counter' ? { actionKey: 'x' } : type === 'sacrifice' ? { amount: 1 } : { effectId: 'x' } } as any, context, 'x', 'kit.effect')).toThrow('reserved effect');
});
