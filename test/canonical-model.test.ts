import { CANONICAL_METADATA_TAGS, EFFECT_TYPES, compareAuthorityV1, directlyConflicts, dispatchEffect, requireCanonicalMetadataTag, resolveDirectConflictV1, type MechanicClaim } from '../src/combat/canonical-model';
...
test('canonical tag and effect registries are closed', () => {
  ...
  expect(() => dispatchEffect({ type: 'heal', payload: {}, target: 'target:ally' }, 'x', 'kit.skill1.effect')).toThrow('no production handler');
});
