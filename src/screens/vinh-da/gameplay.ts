...
export function renderScreen(context: RenderContext): { destroy: () => void }{
  ...
  const renderStatusPanel = (): void => {
    ...
    if (barracks && barracksStat) lines.push(`<div>Trại: ${barracksRuntime?.soldiers?.length ?? 0}/${barracksStat.soldierCap ?? 0} lính · rank ${barracksStat.soldierRankName ?? barracksStat.soldierRank ?? 1}${barracksStat.requiresCollectionPick ? ' · collection/fallback' : ''} · ulti ${barracksStat.ultimatePermission ? 'ready' : 'khóa'}</div>`);
