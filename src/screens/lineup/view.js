...
function ensureStyles(){
  ...
  const css = `
    ...
    .lineup-leader{border-radius:26px;border:1px solid rgba(255,209,132,.42);background:linear-gradient(150deg,rgba(36,26,12,.88),rgba(18,12,6,.92));padding:24px;display:grid;grid-template-columns:minmax(0,1.2fr) minmax(0,2fr);gap:20px;align-items:start;position:relative;overflow:hidden;}
    .lineup-leader__badge{position:absolute;top:16px;right:-32px;background:rgba(255,209,132,.16);color:#ffd184;padding:6px 40px;border-radius:999px;font-size:12px;letter-spacing:.18em;text-transform:uppercase;transform:rotate(20deg);}
    .lineup-leader__main{display:flex;flex-direction:column;gap:12px;}
    .lineup-leader__avatar{width:120px;height:120px;border-radius:28px;background:rgba(54,36,18,.9);display:flex;align-items:center;justify-content:center;font-size:40px;color:#ffd184;overflow:hidden;cursor:pointer;transition:transform .18s ease,box-shadow .18s ease;border:1px solid rgba(255,209,132,.45);}
    ...
    .lineup-leader__name{margin:0;font-size:20px;color:#ffe7b3;}
    .lineup-leader__note{margin:0;font-size:13px;color:#f0d9b2;line-height:1.5;}
    .lineup-passives{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:12px;}
    .lineup-passive{padding:12px;border-radius:14px;border:1px solid rgba(255,209,132,.28);background:rgba(38,26,12,.78);display:flex;flex-direction:column;gap:8px;cursor:pointer;transition:transform .16s ease,border-color .16s ease,background .16s ease;color:#ffe7b3;}
  ...
  function renderBench(){
    ...
    lineup.bench.forEach(cell => {
      ...
      if (cell.unitId){
        ...
        renderAvatar(avatarEl, unit?.avatar || cell.meta?.avatar || null, unit?.name || cell.label || '');
      } else {
      ...
      if (cell.unitId){
        const unit = rosterLookup.get(cell.unitId);
        nameEl.textContent = unit?.name || cell.label || 'Đã gán';
      } else if (cell.label){
        nameEl.textContent = cell.label;
      } else {
