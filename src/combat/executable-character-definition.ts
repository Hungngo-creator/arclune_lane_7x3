...
function effectsFor(action: Record<string, unknown>, target: TargetSpec): EffectSpec[] {
  ...
  // Catalog actions without a numeric payload still have a real runtime adapter/state behavior.
  if (effects.length === 0) effects.push({ type: 'set-stance', target: { kind: 'self' }, payload: { value: String(action.key ?? 'active') } });
  return effects;
}
