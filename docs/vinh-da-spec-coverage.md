...
| Church / nhà thờ | **Partial** | `src/screens/vinh-da/structures.ts`, `src/screens/vinh-da/simulation.ts` | Có buff chỉ số, healing bonus, prayer heal và cleanse contamination đúng timer 2 giờ lore (`cleanseContaminationSeconds = 120`). Thiếu/đơn giản hóa: phạm vi buff theo lãnh địa cũ/mới khi base dời map, cap healing tổng hợp với base/mộc/thủy/kiến trúc khác, logic NPC đến nhà thờ sau wave và exception cho Sứ Đồ. |
