...
function extractLeftExpression(code, pos){
  ...
  let start = i + 1;
  while (start < pos && /\s/.test(code[start])) start++;
  const keywords = ['return', 'throw', 'yield', 'await', 'case'];
  let adjusted = true;
  while (adjusted){
    adjusted = false;
    for (const kw of keywords){
      if (code.startsWith(kw, start)){
        const nextCh = code[start + kw.length] || '';
        if (!/[_$A-Za-z0-9]/.test(nextCh)){
          start += kw.length;
          while (start < pos && /\s/.test(code[start])) start++;
          adjusted = true;
        }
