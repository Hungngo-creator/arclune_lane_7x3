...
const mapTarget = (tsTarget) => ScriptTarget[tsTarget] || "esnext";
  ...
  if (esbuild && !esbuild.__arcStub && typeof esbuild.transformSync === "function") {
    ...
    const stripTypeOnlyImports = (source) => {
      return source
        .split("\n")
        .filter((line) => !/^\s*import\s+type\s+/u.test(line))
        .join("\n");
    };

    const removeTypeAnnotations = (source) => {
      return source
        .replace(/:\s*[^,)=>{]+(?=[,)=>{])/gu, "")
        .replace(/<[^>]+>(?=\s*[({])/gu, "");
    };

    const stripDeclareStatements = (source) => source.replace(/\bdeclare\s+[^;]+;?/gu, "");

    const stripInterfacesAndTypes = (source) =>
      source.replace(/\binterface\s+[^{]+{[^}]*}/g, "").replace(/\btype\s+[^{=]+=[^;]+;/g, "");

    transpileModule = (input) => {
      let output = input;
      output = stripTypeOnlyImports(output);
      output = stripInterfacesAndTypes(output);
      output = stripDeclareStatements(output);
      output = removeTypeAnnotations(output);
