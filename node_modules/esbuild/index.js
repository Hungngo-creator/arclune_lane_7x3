...
function loadTypeScript(){
  try {
    ...
    try {
      const resolved = require.resolve('typescript', { paths: [globalModules, process.cwd()] });
      return require(resolved);
    } catch (err2){
      const originalError = err2 ?? err;
      const warning = [
        '[esbuild-stub] Không thể nạp module "typescript". Các loader TS sẽ hoạt động ở chế độ passthrough.',
        originalError && originalError.message ? `Nguyên nhân: ${originalError.message}` : null,
      ].filter(Boolean).join(' ');
...
const DEFAULT_TARGET = typescript ? typescript.ScriptTarget.ES2023 : undefined;
...
async function transform(code, options = {}){
  ...
  if (typescript && (loader === 'ts' || loader === 'tsx')){
    ...
    return { code: transpileResult.outputText, map: transpileResult.sourceMapText ?? null, warnings: [] };
  }
  return { code, map: generateMap ? createIdentitySourceMap(code, sourcefile) : null, warnings: [] };
}
 ...
 async function build(options = {}){
  ...
  const result = {
    ...
    warnings: [],
    outputText,
    sourceMap: transformed.map ?? null,
  };
  if (metafile){
